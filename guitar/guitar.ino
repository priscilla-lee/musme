...
//define notes
const byte none[] = {64, 59, 55, 50, 45, 40}; //40, 45, 50, 55, 59, 64, 88};
const byte    G[] = {67, 59, 55, 50, 47, 43}; //43, 47, 50, 55, 59, 67, 91};
const byte   Em[] = {64, 59, 55, 52, 47, 40}; //40, 47, 52, 55, 59, 64, 88};
const byte    C[] = {64, 60, 55, 52, 48, 40}; //40, 48, 52, 55, 60, 64, 88};
const byte    D[] = {66, 62, 57, 50, 45, 40}; //40, 45, 50, 57, 62, 66, 90};
...
void loop(){
   if(MPR121.touchStatusChanged()){
    //Serial.println("touched");
     ...
     //get chord input
     for (int i = firstChord; i <= lastChord; i++) {
        if(MPR121.isNewTouch(i)) {
          //clear chords if new one is being touched
          for (int i = 0; i < 4; i++) {
            chords[i] = 0;
          }
          chords[i-firstChord] = 1; //update array (chord being touched)
        } else if (MPR121.isNewRelease(i)) {
          chords[i-firstChord] = 0; //update array (chord released)
        }
     } 

     //strum strings
     for(int i=firstString; i<=lastString; i++){
       //retrieve correct chord
       chord = getChord();
       if (chord == 0) { //G chord
        //Serial.println("G");
        note = G[i-firstString];
       } else if (chord == 1) { //Em chord
         //Serial.println("Em");
        note = D[i-firstString];
       } else if (chord == 2) { //C chord
         //Serial.println("C");
        note = C[i-firstString];
       } else if (chord == 3) { //D chord
         //Serial.println("D");
        note = Em[i-firstString];
       } else { //empty chord
         //Serial.println("open");
        note = none[i-firstString];
       }
...
void setupMidi(){
  ...
  talkMIDI(0xC0, 26, 0); // Set instrument number. 0xC0 is a 1 data byte command(55,0) 
  // ---------------------------------------------------------------------------------------------------------
